...
var logEntryKvTemplate = (prefix, timestamp, workerId, uniqueId, attemptSuffix) => `${prefix}${timestamp}-${workerId}-${uniqueId}${attemptSuffix}`;
  ...
  return {
    ...
    output: async (entry) => {
      ...
      try {
        ...
        if (Math.random() < 0.02)
          cleanupOldLogsIfNeeded(kv, logsLimit, logsTrigger, keyPrefix).catch(() => {});
      } catch (_error) {}
    }
  };
}
...
async function cleanupOldLogsIfNeeded(kv, logsLimit, logsTrigger, keyPrefix) {
  try {
    ...
    if (listResult.keys.length <= logsTrigger)
      return;
